...
import { CREATE_ITEM, DELETE_ITEM, GET_ITEMS } from "@/queries/queries";
...
export default function UppyDashboard({ id, allowedFileTypes, dependencies, onSelect, preselectedFile }: {
  ...
  const [deleteItemMutation, deleteItemMutationResult] = useMutation<{
    id: string;
  }>(DELETE_ITEM("files_default_context"), {
    onCompleted: () => {
      // todo refetch items
    }
  })
  ...
  const uppy = useUppy(
    {
      ...
      callbacks: {
        uploadSuccess: async (data) => {
          ...
          // todo create item in files context
          const item = {
            name: data.file.name,
            type: data.file.type,
            s3bucket: configContext?.s3Bucket,
            s3region: configContext?.s3region,
            s3endpoint: configContext?.s3endpoint,
            s3key: data.key,
            url: data.url
          }
          await createItemMutation({ variables: { input: item } })
          // filesQuery.refetch() // todo replace with items fetch
        },
      },
      maxNumberOfFiles: 10,
    },
        ...
        <div className="border rounded-lg p-4">
          ...
          <ScrollArea className="h-[400px] pr-3">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {data?.files_default_context_itemsPagination?.items?.map((file: Item) => (
                <div key={file.s3key} onClick={() => {
                  onSelect(file.s3key)
                  setSelectedFile(file.s3key)
                  setOpen(false)
                }} className={`group relative rounded-lg p-2 hover:bg-muted transition-colors cursor-pointer ${selectedFile === file.s3key ? 'border border-purple-500' : 'border'}`}>
                  <div className="aspect-square relative mb-2 bg-muted/50 rounded-md overflow-hidden flex items-center justify-center">
                    {(
                      file.s3key.toLowerCase().endsWith("jpg") ||
                      file.s3key.toLowerCase().endsWith("jpeg") ||
                      file.s3key.toLowerCase().endsWith("png") ||
                      file.s3key.toLowerCase().endsWith("svg")
                    ) ? (
                      <SecureImageRenderComponent fileKey={file.s3key} />
                    ) : (
                      <div className="flex flex-col items-center justify-center h-full">
                        {getFileIcon(file.s3key)}
                        <span className="text-xs text-muted-foreground mt-1">{file.s3key.split("-").pop()}</span>
                      </div>
                    )}
                  </div>
                  <p className="text-xs truncate">{file.s3key.split("-").pop()}</p>
                  <div className="opacity-0 group-hover:opacity-100 flex absolute top-1 right-1">
                    <Button
                      ...
                      onClick={() => {
                        files.download(file.s3key).then(async res => {
                          console.log("res", res);
