...
export function classifyRows(
  ...
  return rows.map((row) => {
    ...
    const id =
      typeof row.cells.id?.value === "string" ? row.cells.id.value : "";
    const ext =
      typeof row.cells.external_id?.value === "string"
        ? row.cells.external_id.value
        : "";
    if (id) {
      if (seenIds.has(id)) {
        return {
          ...next,
          error: { code: "duplicateKey", params: { field: "id" } },
        };
      }
      seenIds.add(id);
      if (!existing.knownIds.has(id))
        return { ...next, error: { code: "idNotFound" } };
    ...
    if (ext) {
      if (seenExternal.has(ext)) {
        return {
          ...next,
          error: { code: "duplicateKey", params: { field: "external_id" } },
        };
      }
