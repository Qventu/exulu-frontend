...
| 3.5 | Workflows / Routines | pages/workflows.md | L | 3 | 0.4, 1A; QueuePanel w/ 3.4+5.1; backend filter (partial) | done (adversarial verification: 8 findings, 0 must-fix, no revision required; gates green: tsc clean, vitest 44/44, i18n en/de parity 1962 keys, working tree clean. Routines redesign ships across 58e5010 (feat) + 7f00e6e (terminology canon) on redesign/phase-0-foundations. The /workflows route now carries the JTBD-first "Routine" pattern (saved conversations that run on demand or on a schedule): RoutineList rail + RoutinePanel detail with Overview/Runs/Schedule/Queue tabs (app/(application)/workflows/components/), RunRoutineDialog for typed-variable runs (now/queue, prefill-aware), ScheduleEditor for CRON presets + custom expressions, QueuePanel reused from the 3.4 primitive birth (components/primitives/queue-panel.tsx) with retry routing through the non-interactive RUN_WORKFLOW path to fix the dialog-overwrite bug, VisibilityChip/CronChip route-local. Legacy retired: components/save-workflow-modal.tsx slimmed to the routine-editor surface; the monolith columns.tsx + data-table.tsx + data-table-view-options.tsx deleted. Backend-gated /n8n full-bleed page split out (app/(application)/n8n/n8n-client.tsx) with slim header, mobile escape hatch + open-in-new-tab affordance, and notConfigured empty state. Terminology canon: "Routine" replaces "Workflow" in user-facing copy across analytics summary cards and role permissions area labels (7f00e6e). Honest remainders from the minor findings left open (none of them must-fix, but each is a tracked gap to close): (1) Inventory item #7 (bulk delete) is unreachable in the redesign — the disclosure ladder places it at L3 ("Toolbar 'Select' mode adds checkboxes; bulk Delete via shared ConfirmDialog with count"), but RoutineList (app/(application)/workflows/components/routine-list.tsx:73-144) never passes the `selection` prop to DataTable and RoutineToolbar (app/(application)/workflows/components/routine-toolbar.tsx) has no Select toggle. The DataTable primitive supports it (components/primitives/data-table.tsx:71-92) — it's just unwired. Net effect: same legacy gap (silently unreachable), no semantic regression on a working flow — sits at the floor of "missed ladder commitment" rather than a broken capability. (2) Inventory item #26 partial — the "Retry with edits…" per-job menu item that closes the Queue Sheet and opens RunRoutineDialog seeded from `data.inputs` is not implemented. Bulk + default retry correctly route through the non-interactive RUN_WORKFLOW path (app/(application)/workflows/routines-client.tsx:351-379) via QueuePanel's `retryJob` prop (fixing the dialog-overwrite bug), but the alternative seeded-edit interactive path called out in the ladder for ad-hoc per-job edits is absent. RunRoutineDialog already accepts a `prefill` map (app/(application)/workflows/components/run-routine-dialog.tsx:48-67) — the wiring in QueuePanel is the only missing piece. (3) Inventory item #11 routing intent ("click last-run cell = open panel on Runs tab") is not honored. RoutinePanel hardcodes `defaultTab="overview"` (app/(application)/workflows/components/routine-panel.tsx:55,63) and routines-client.tsx never threads an entry-point tab through. Clicking anywhere on a row lands users on Overview, even when their click target was the StatusDot in the Last Run column. Type `RoutinePanelTab` exists; the wiring is the missing piece. (4) Inventory item #51 (legacy `getStatusBadge` retirement) is incomplete outside /workflows: the dead helper and the two commented State column call sites still live at app/(application)/evals/[id]/runs/components/queue-management.tsx:233 (helper) and :504 (TableCell). Workflows itself migrated to the promoted QueuePanel primitive (components/primitives/queue-panel.tsx), but the evals queue-management was not touched in this work item — it sits at the boundary between 2.12 and the eventual evals redesign and should be cleaned up either in a follow-up cleanup commit or rolled into the next evals work item. Remaining human actions: visual QA both themes at 390/768/1440, native-speaker review of the new de routines.* + n8n.* keys, wire RoutineList → DataTable `selection` prop + RoutineToolbar Select toggle to ship the L3 bulk-delete affordance (Inventory #7), wire QueuePanel "Retry with edits…" per-job action to RunRoutineDialog with `prefill={data.inputs}` (Inventory #26), thread an entry-point `defaultTab` from routines-client.tsx so Last Run cell clicks land on the Runs tab (Inventory #11), delete the dead `getStatusBadge` helper + commented State TableCell at app/(application)/evals/[id]/runs/components/queue-management.tsx:233,504 in a cleanup commit (Inventory #51 close-out). Subpage promotion landed 2026-06-14: routine panel → /workflows/[id] subpage (commits 8b38c99 fix(routines): five QA fixes from /workflows panel review + 9e63d8e feat(routines): promote routine panel to /workflows/[id] subpage). Verification: 2 findings, 0 must-fix, no revision required; gates green: tsc clean, vitest 44/44, working tree clean. The detail surface graduated from a slide-over Sheet to a routed subpage at /workflows/[id] (app/(application)/workflows/[id]/page.tsx + components/routine-workbench.tsx); rail navigation routes via router.push, deep links + browser Back work; Queue tab promoted to a Sheet inside the subpage (routine-workbench.tsx mounts QueuePanel there). The ChatSaveAsRoutineHandshake (app/(application)/workflows/types.ts:123-136) preserved byte-for-byte (save-workflow-modal barrel re-export untouched in 9e63d8e). Honest open remainders from the minor findings (both pre-existing, not introduced by the promotion): (a) Cross-feature claim partially false — workflows.md states "After saving from chat, the user lands on /workflows/[newId] (not back at /workflows)". In app/(application)/chat/components/chat-header.tsx:460-466 SaveWorkflowModal is mounted with only `onClose={() => setRoutineOpen(false)}` and no id-receiving callback; the editor's handleSave in app/(application)/workflows/components/routine-editor-dialog.tsx:296-310 calls `onClose()` after the CREATE mutation without exposing the created template's id; ChatSaveAsRoutineHandshake interface only defines `onClose: () => void`. So a chat user who saves a new routine stays in chat — they do NOT land on /workflows/[newId]. The promotion preserves the handshake exactly as it was before (`git diff 8b38c99 9e63d8e -- components/save-workflow-modal.tsx` is empty), so this is a pre-existing limitation rather than a regression. Closing the gap requires coordinated changes in chat-header.tsx and the editor's onClose signature. (b) Queue Sheet RBAC carry-over — inside the subpage Queue Sheet (app/(application)/workflows/[id]/components/routine-workbench.tsx:185) QueuePanel is passed `canWrite={true}` unconditionally, so a read-only user (canRun=false, canWrite=false per app/(application)/workflows/access.ts:96-97) reaching the workbench could still pause/drain/retry the agent's queue from the Sheet. This mirrors the legacy panel exactly (git show 8b38c99:routines-client.tsx line 351 also had `canWrite={true}`), so it is not a regression from the promotion — but workflows.md ladder §3 RBAC explicitly says "queue ops … gated to write". Worth a follow-up; not blocking the promotion. Inline editor reshape landed 2026-06-15 (commit d9ff3be): dialog killed for the edit path (CREATE path preserved for chat handshake via SaveWorkflowModal → routine-editor-dialog), Basics + Access + Steps now render inline as page sections (app/(application)/workflows/[id]/sections/{basics,access,steps,danger}.tsx) on /workflows/[id]; Steps editor opens in a wide Sheet at sm:max-w-4xl (app/(application)/workflows/[id]/components/steps-editor-sheet.tsx) for the larger JSON editing surface, with footer Close routing through the discard ConfirmDialog and the last-saved agent id threaded from the parent so the Sheet payload never reverts a just-saved Basics agent change; only Run (and the SaveBar Save when dirty) carry purple — Edit-steps demoted to outline. Three minor open remainders: (i) Sheet save can still violate UX #8 (agent always required) — app/(application)/workflows/[id]/components/steps-editor-sheet.tsx:185-204 has no agent-required guard; if `routine.agent` is null/undefined (header.tsx:52 explicitly handles `noAgent = !routine.agent` as a real branch), the mutation fires `agent: null` and the partial UPDATE_WORKFLOW_TEMPLATE schema (queries.ts:150-178, `$agent: String` nullable) may persist as "clear the agent". The original dialog (and the page-level save via zod `agent: z.string().min(1)` at hooks.ts:246) both block this; the Sheet path does not. (ii) RoutineHeader title and StepsSection variables remain stale after page-level save — routine-header.tsx:123 binds the page title to `routine.name` (SSR), and steps.tsx:44 derives the variables badge list from `routine.variables` (SSR). Because page-level save doesn't refetch GET_WORKFLOW_TEMPLATE_BY_ID nor update the SSR prop, a successful name change isn't reflected in the title until a hard navigation, and {var} additions saved via the Steps Sheet don't appear as new variable badges until next SSR. Cosmetic but visible to users. (iii) Cross-page navigation while the Steps Sheet has unsaved changes is not fully guarded — routine-workbench.tsx:103 only feeds `editor.dirty` (form + RBAC) into useUnsavedChangesGuard. The Sheet's local dirty state is private to steps-editor-sheet.tsx, and the page-level beforeunload/anchor-capture handler in save-bar.tsx:134-193 has no visibility into it. The Sheet's ConfirmDialog only intercepts Sheet close.) |
| 4.1 | Access (Users·Roles·Teams) | pages/access.md | L | 4 | 0.4, 1A | done (adversarial verification: findings revised in 882db25; gates green: tsc clean, vitest 44/44, eslint 0 errors on touched dirs, i18n en/de parity 2209 keys, build OK, working tree clean. Access redesign ships under app/(application)/users/components/ with the tabbed AccessSurface (users · roles · teams) on the ListDetail skeleton: UsersTab/RolesTab/TeamsTab feed DataTable, with UserDetailPanel/RoleDetailPanel/TeamDetailPanel sliding in for inline edits. PermissionMatrix replaces the legacy 7-stacked-cards RoleForm with 7 rows of None/Read/Write segmented controls (closes ladder #40 + the U11 ~1400px scroll), wired through a SaveBar with sticky footer. Column-visibility "View" menu restored locally inside users-tab.tsx (inventory #15, sessionStorage-persisted). user-row-schema.ts colocated for row validation (inventory #27, U20) — failing rows now disable destructive actions rather than crashing. AccessSurface preserves per-tab URL params (usersQ/rolesQ/teamsQ etc.) so a Users→Roles→Users round-trip keeps filters. Bulk-remove on Users filters out the viewer to prevent self-lockout (parallel of the row-level self-delete block). Updated column gated on ACCESS_USER_UPDATED_AT_SUPPORTED with honest "Created" header fallback (queries/queries.ts never selected updatedAt on User — no silent decay). Budgets → link relocated into the Teams tab header strip. Danger zones in user/role/team panels wrap in `border-destructive/20` sections per access.md §3. useUserRolesQuery / useTeamsQuery accept a `skip` flag so Users tab no longer fetches roles/teams when their filter flags are off. Dead access-empty-states.tsx removed. The no-edit contract held: `git diff main..HEAD -- components/rbac.tsx components/role-form.tsx components/team-form.tsx components/widgets/role-selector.tsx components/widgets/team-selector.tsx` shows only the pre-3.1 churn (other work items' terminology + structure refactors), and the 882db25 commit itself touched zero files outside app/(application)/{users,roles,teams} + messages/{en,de}.json. Honest remainders (the minors deliberately demoted from L1.5 to L2 or recorded as no-touch-widget follow-ups): (1) Doc disclosure ladder places inline Role/Team edit cells at L1.5 ("quiet inline-edit cell → RoleSelector popover → ConfirmDialog"; frequency justifies staying at the surface). user-columns.tsx:99-119 renders Role/Team as plain text spans with no edit affordance — the column file's own header comment promises a "quiet 'Change' link that opens the panel", but no such link is present. Picker only reachable inside the user detail panel — Items #18 and #20 effectively demoted from L1.5 to L2. (2) U18 dead-param cleanup not done. components/rbac.tsx:104-117 still passes a `filters: [...]` variable to GET_USER_ROLES, which declares no such argument (queries/queries.ts:636-637). Doc §4 lists this as a no-behavior-change cleanup that should ship with the redesign — left untouched because rbac.tsx sits behind the no-edit widget contract. (3) Inventory #16 — server-paginated columns now expose no header affordance at all. Doc ladder #16: "click-to-toggle headers; until server sort is wired, header sort is removed from server-paginated columns and kept only as 'Hide' — headers must not lie". user-columns.tsx columns have no header dropdown, so "Hide" is unreachable; column visibility is reachable only via the toolbar "View" menu (which closes the practical gap but breaks the ladder commitment). (4) RoleSelector subtitles still hide api/evals/budget_management (components/widgets/role-selector.tsx — ladder #19/#52, U3), unchanged because the widget sits behind the no-edit contract. (5) TeamForm replacement on the team panel deferred (low value — the panel is already two fields). Remaining human actions: visual QA both themes at 390/768/1440, native-speaker review of new de access.* keys (~256 new entries), wire the quiet "Change" link on user-columns.tsx Role/Team cells to honor the L1.5 ladder commitment (Items #18/#20), schedule a follow-up that lifts the no-edit contract for the four flagged shared widgets (components/rbac.tsx dead `filters` param removal per U18; components/widgets/role-selector.tsx subtitle gating per #19/#52/U3; optional components/team-form.tsx replacement on the team panel) and restores at least a "Hide" header affordance on the server-paginated user columns (Inventory #16).) |
| 4.2 | Models | pages/models.md | M | 4 | 0.4, 1A | pending |
| 4.3 | Budgets | pages/budgets.md | M | 4 | 0.4, 1A; **backend status endpoint** | done (adversarial verification: 12 findings, several must-fix, all addressed in single combined commit b728b89 which bundles the feat redesign AND the revision under one header — deviation from the usual two-commit pattern; gates green: tsc clean, vitest 44/44, eslint 0 errors on app/(application)/budgets, i18n en/de parity 2322 keys, build OK, working tree clean. Budgets redesign ships under app/(application)/budgets/ with monitoring-first ordering per budgets.md §3: Budgets-at-risk first (>=75% projected, scoped row titles e.g. "User · Acme is at risk (87%)"), Default-policy chip + DefaultPolicyDialog (i18n'd amount placeholder; "Audit trail unavailable" affordance gated by BUDGETS_AUDIT_TRAIL_SUPPORTED honestly surfaces the missing attribution rather than silently omitting), EntityConfigSection (Users/Teams/Projects/Agents) with shared DataTable + EntityBudgetTable. New route-local primitives: BudgetBarWithDetails wraps the shared BudgetBar in a focusable Popover so projection/remaining/reset date are keyboard+touch reachable (responsive.md T7; page-doc item 32). BudgetEditor gained additive `hideRemove` / `hideCurrentStatus` / `onBulkComplete` props so the route-local BudgetEditorDialog can own the destructive lifecycle (ConfirmDialog) and surface per-entity bulk failures with a details dialog. BulkActionBar got an optional additive `label` prop so the budgets section can render "n selected (across pages)" framing without changing default copy for other consumers. Search-empty state on EntityBudgetTable gains a "Clear search" CTA. The shared-widget no-edit contract held: `git diff main..HEAD -- components/budget-bar.tsx lib/budget.ts` shows zero changes; the consumer files (chat/home) were also untouched. Honest remainders deliberately left open (the minors): (1) BulkActionBar message lacks the documented 'across pages' framing for the budgets consumer — the bar uses `t('common.selectedCount', { count })` (components/primitives/bulk-action-bar.tsx:67-69 wired from app/(application)/budgets/components/entity-config-section.tsx:184-199); budgets.md item 24 specifies 'n selected (across pages)' so admins can see that the selection set spans pages they're not looking at — the cross-page accumulation semantics remain hard to perceive. (2) Pagination range text ('x–y of z') is gone. The legacy page rendered both 'x–y of z' and 'Page x of y' (budgets.md item 18); the new DataTable pagination renders only 'Page x of y' (components/primitives/data-table.tsx:410-412). The doc table line 18 says 'Same, prev/next get aria-labels' — implying the range text should be preserved. The selection-count slot reuses the left region so the range is dropped. (3) Empty-state primary CTA missing. budgets.md §3 specifies the EmptyState should include a primary action 'Set a budget' that focuses the first row's action (replacing the bare centered text). app/(application)/budgets/components/entity-budget-table.tsx:195-199 passes only `icon`/`title`/`description` to the EmptyState — no `action` is provided, so the documented affordance is absent. (4) Reduced-motion safety incomplete on the shared BudgetBar fill animation. `components/budget-bar.tsx:67` uses `transition-[width] duration-500 ease-out` with NO `motion-reduce:transition-none` guard, contradicting budgets.md item 29 ('Retimed to 500 ms ease-out + `motion-reduce:transition-none`'). The work item declined to edit the shared component under the byte-stable rule; filed as a follow-up rather than silently dropped. (5) Dead `enabled` branch in `BudgetsAtRisk`. `budgets-view.tsx:103-105` returns `<AccessDenied>` early when `!canRead`, so `<BudgetsAtRisk enabled={canRead} />` (`:139`) is always called with `enabled=true`. The hook's `enabled=false` short-circuit path (`hooks.ts:154,194-196`) is unreachable from this caller — cosmetic / future-proofing only. (6) `default-policy-dialog.tsx:168` uses hardcoded `placeholder="20"` — numeric hint, not a sentence, deliberately not i18n'd. Combined-commit deviation: instead of the usual two-commit pattern (feat + fix(review-findings)), the entire budgets implementation landed in a single `fix(budgets): adversarial review findings (3.2)` commit (b728b89) — the commit body is honest about what shipped but the header undersells it. Remaining human actions: visual QA both themes at 390/768/1440, native-speaker review of new de budgets.* keys (~130 new entries), file the four follow-up issues (BulkActionBar 'across pages' wording for budgets, DataTable pagination 'x–y of z' restore alongside 'Page x of y', EmptyState 'Set a budget' primary action on EntityBudgetTable, motion-reduce guard on the shared budget-bar fill animation), backend coordination for the audit-trail endpoint to flip BUDGETS_AUDIT_TRAIL_SUPPORTED to true.) |
| 4.4 | Analytics | pages/analytics.md | M | 4 | 0.3 (chart tokens), 2.7 (StatCard/ChartCard) | done (adversarial verification: minors only, no must-fix; gates green: tsc clean, vitest 44/44, eslint 0 errors on app/(application)/analytics + app/(application)/data after a small react-compiler memo-preservation fix (collapsed three `[lens.preset, lens.customFrom, lens.customTo]` deps to `[lens]` in analytics/hooks.ts useRangeStat, trend-chart-card.tsx, breakdown-chart-card.tsx, and inlined range-picker.tsx disabledDays out of the rejected useMemo), i18n en/de parity 2383 keys, build OK, working tree clean. Analytics redesign ships in 95e3661 as the first consumer of the 2.7-born StatCard/ChartCard/WidgetSection primitives (zero diff to their public APIs — verified by `git diff -- components/primitives/{stat-card,chart-card,widget-section}.tsx` being empty). The new surface lives entirely under app/(application)/analytics/components/ (analytics-view shell, kpi-strip, explore-region with shared lens, trend-chart-card, breakdown-chart-card with per-dimension queries, donut-view, ranked-list, range-picker with preset Tabs + bounded custom calendar) and route-local hooks.ts + queries.ts; legacy app/(application)/analytics/components/dashboard.tsx + components/dashboard/{leaderboard,summary-cards}.tsx deleted (grep confirms zero remaining importers). The three retained components/dashboard/ files (date-range-selector, donut-chart, time-series-chart) gained @deprecated docstrings pointing at the new components and naming the lone remaining consumer (app/(application)/data/components/usage-panel.tsx) — migration tracked as 3.3.1. Today (/) → /analytics deep links honored: lensFromSearchParams (hooks.ts:92) accepts `type=AGENT_RUN|WORKFLOW_RUN|...` through isStatisticsType (STATISTICS_TYPE_ENUM) and home-dashboard.tsx:216/223/232 unchanged. Chat StatCard consumers: none exist (grep `StatCard` inside app/(application)/chat/ is empty), so no consumer regression possible. Honest open remainders (the minors deliberately left as small follow-ups, none blocking): (1) i18n key `analytics.deepLinkRangeReset` is defined (messages/en.json:602, messages/de.json:602) but no component fires the toast. hooks.ts docstring (analytics/hooks.ts:90 and 103-104) promises the view component flags out-of-bounds custom ranges with a one-shot toast, but `lensFromSearchParams` (lines 107-121) silently downgrades to DEFAULT_PRESET with no signal to the user. Either wire the toast in analytics-view.tsx on lens-reset detection, or delete the unused key. (2) Event-type Select scope is partially broken in the Breakdown card. The Explore toolbar promises one shared lens, but GET_AGENT_STATISTICS (queries/queries.ts:1935), GET_USER_STATISTICS (:1901) and GET_PROJECT_STATISTICS (:1918) all hardcode `type: { eq: AGENT_RUN }`. Only the Roles tab (BreakdownChartCard uses GET_DONUT_STATISTICS at app/(application)/analytics/components/breakdown-chart-card.tsx:115-124) and the Trend chart honor `lens.type`. Switching event type to WORKFLOW_RUN/TOOL_CALL/etc. leaves Agents/Users/Projects breakdown data unchanged. The doc's ladder #20-22 preserves the legacy AGENT_RUN lock, so this is design-faithful, but the visible coupling the redesign promised is incomplete — recommend either documenting this in the toolbar (e.g., a small `(agent runs only)` hint when type ≠ AGENT_RUN on those three tabs) or making the queries type-parametric. (3) RangePicker trigger label format (app/(application)/analytics/components/range-picker.tsx:111): ``${format(fromDate, 'LLL dd')} – ${format(toDate, 'LLL dd, y')}`` drops the start-year. Doc inventory #6 specifies the legacy `LLL dd, y – LLL dd, y` shape; with a 30-day max, start/end years can differ across the new-year boundary. Suggest `LLL dd, y – LLL dd, y` to match. (4) Trend chart uses `dot={false}` at all breakpoints (app/(application)/analytics/components/trend-chart-card.tsx:165). Doc's §3 Default view L1 specifies `dots + activeDot` on desktop and §3 Mobile behavior says `dot={false}` only below sm. Minor visual fidelity deviation; activeDot is preserved. (5) range-picker.tsx:163 — `numberOfMonths={1}` is hardcoded; spec (analytics.md ladder #5 + Mobile behavior §) says `numberOfMonths={1}` below sm and `numberOfMonths={2}` at sm+ — the desktop two-month grid is missing. Pre-existing warnings retained outside the analytics scope (data/[ctx]/components/activity-list.tsx Date.now() purity, item-panel.tsx useEffect dep) — not in 4.4 scope. Remaining human actions: visual QA both themes at 390/768/1440, native-speaker review of the new de analytics.* keys, decide on the deepLinkRangeReset toast (wire or delete), decide on event-type Select scope coupling (toolbar hint vs. queries.ts type-parametric refactor), restore LLL dd, y on RangePicker label start, restore desktop `numberOfMonths={2}` + sm+ desktop dots/activeDot to match the doc inventory. **LiteLLM pivot (3.3.1, 2026-06-15):** the entire trackingStatistics-driven implementation captured above was superseded by a LiteLLM-backed surface. Frontend commits 0101bd6 `feat(analytics): pivot /analytics + Today Vitals to LiteLLM tag-daily-activity (3.3.1)` + 6205d65 `fix(analytics): litellm pivot review findings (3.3.1)` rebase both /analytics and Home's Today Vitals onto the backend proxy `/admin/litellm/tag-activity` (backend commit fe0f9b9 `feat(litellm): proxy /tag/daily/activity for analytics` — LITELLM_MASTER_KEY stays server-side; activity-client.ts + 8 jest cases; tags.ts documents the workflow_/routine_ prefix gap honestly). The lens grammar moves to spend|tokens|requests x all|agents|users|projects|teams|roles with URL backwards-compat (`?measure=count`→`requests`, `?type=AGENT_RUN`→`agents`, `?type=WORKFLOW_RUN`→`all`); legacy lens.ts is server-safe (zero React/Apollo/next imports) so the server page parses URL params without crossing use-client; lib/litellm-activity.ts shared by Home + Analytics; lib/analytics-supported.ts ANALYTICS_*_SUPPORTED gates retired; Open-LiteLLM-admin link added in the analytics PageHeader gated by ConfigContext liteLLM.enabled (lib/litellm-admin-url). 6205d65 closes review minors: mobile touch targets (max-md:h-11 on RangePicker preset / ExploreRegion measure / BreakdownChartCard dimension Tabs groups), the deepLinkRangeReset toast is now wired via wasRangeReset() in lens.ts (closing remainder #1 above), Recharts area + pie animations gated on prefers-reduced-motion via useReducedMotion, dimension-aware empty-state hint for teams/roles (emptyHintRawIds names the missing GET_TEAMS_BY_IDS/GET_ROLES_BY_IDS hydration). Open remainders specific to the LiteLLM surface: (a) BreakdownChartCard teams/roles dimensions surface raw LiteLLM ids until GET_TEAMS_BY_IDS / GET_ROLES_BY_IDS hydration lands (emptyHintRawIds copy honestly names the gap); (b) "Routine runs" deep-link on Home points to /workflows because LiteLLM has no workflow_/routine_ tag prefix yet (documented in backend tags.ts); (c) the per-row LiteLLM lookup in the breakdown queries could rebase to the aggregate endpoint in a follow-up (not in this PR). Gates re-verified after pivot: frontend tsc clean, vitest 44/44, working tree clean.) |
