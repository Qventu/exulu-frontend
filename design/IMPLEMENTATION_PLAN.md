...
| 2.3 | Chat | pages/chat.md | XL | 2 | 0.4, 1A (AppNavTrigger) | done (adversarial verification 2026-06-11: 18 findings, 2 must-fix — both fixed in revision a2fa2ea; gates green: tsc, vitest 44/44, eslint 0 errors, i18n parity 733 keys, build. The Quiet Column shipped fully route-colocated: app/(application)/chat/components/ + hooks.ts + queries.ts; session-files panel and prompt/save-preset modals moved out of global components/ into the route; the three legacy delete patterns unified on ConfirmDialog; tool-call-approval now tokens-only. 1A deferral closed: MobileTopbar suppression on /chat/[agent]/[session] restored — ChatHeader mounts AppNavTrigger leftmost per navigation.md §5.3. Note for 4.6: message-renderer.tsx only received theme-token fixes here; the read-only replay contract is still open and lands with the feedback console. Minor findings left open: Save-as-Routine item is disabled pre-exchange per ladder row 30 but lacks the specified tooltip explaining why (chat-header.tsx); /chat agent search is a bespoke Input rather than the shared Toolbar primitive chat.md §3 assigns (agent-grid.tsx, behavior intact); below md on /chat/[agent]/search the ChatShell is h-dvh while MobileTopbar still renders, leaving a bar-height double-scroll (topbar suppression is session-routes-only); header Files chip renders a label-only "Files" state while sessionFilesCount is null — it flashes on zero-file sessions and sticks permanently if the files list call fails, softening ladder row 64's "appears when files exist"; bottom safe-area inset is applied twice around the composer (session-screen wrapper + composer root/read-only bar) while the loading skeleton applies it once, causing a loading→loaded shift; ladder row 72's third managed-context placement — the one-line note inside the Context modal — was not implemented (+-menu subtitle and dismissible composer hint shipped). Remaining human actions: visual QA both themes at 390/768/1440, native-speaker review of the 319 new de keys) |
