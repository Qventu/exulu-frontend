...
import { GET_MODELS, REMOVE_MODEL_BY_ID } from "@/queries/queries";
...
export function DataTable<TData, TValue>({
  ...
  const { loading, data, refetch, previousData } = useQuery(GET_MODELS, {
    fetchPolicy: "no-cache",
    nextFetchPolicy: "network-only",
    variables: { page, limit: 10, filters },
    pollInterval: 30000,
  });
  ...
  const [removeModel, removeModelResult] = useMutation(REMOVE_MODEL_BY_ID, {
    refetchQueries: [GET_MODELS, "GetModels"],
  });

  let items;
  let pageCount;
  if (loading && previousData?.modelsPagination?.items) {
    items = previousData.modelsPagination.items;
    pageCount = previousData.modelsPagination.pageInfo?.pageCount;
  } else if (data?.modelsPagination?.items) {
    items = data.modelsPagination.items;
    pageCount = data.modelsPagination.pageInfo?.pageCount;
  } else if (previousData?.modelsPagination?.items) {
    items = previousData.modelsPagination.items;
    pageCount = previousData.modelsPagination.pageInfo?.pageCount;
  }
              ...
              <Button
                onClick={() => {
                  ...
                  table.getSelectedRowModel().rows.forEach((row: Row<any>) => {
                    promises.push(
                      removeModel({ variables: { id: row.original.id } }),
                    );
                  });
                ...
                variant="secondary"
                disabled={removeModelResult.loading}
              >
                {removeModelResult.loading ? (
                  <Loading />
                ) : (
        ...
        <div className="flex items-center space-x-6 lg:space-x-8">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              disabled={!data?.modelsPagination?.pageInfo.hasPreviousPage}
            >
            ...
            <Button
              ...
              disabled={!data?.modelsPagination?.pageInfo.hasNextPage}
            >
